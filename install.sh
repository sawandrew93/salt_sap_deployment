...
if su - $SID_USER -c "HDB version" &>/dev/null; then
    ...
    su - $SID_USER -c "HDB version"  # This will display the version info
else
...
EOF"
    ...
    if [[ $? -eq 0 ]]; then
        echo "User ${NEW_DB_USER} created and privileges granted successfully."
    else
        echo "Error creating user ${NEW_DB_USER}."
        exit 1
    fi
fi
